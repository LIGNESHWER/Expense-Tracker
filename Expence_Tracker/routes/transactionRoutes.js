...
router.post('/delete-all', async (req, res, next) => {
  try {
    ...
    // Verify user ID is valid before converting
    ...
    console.log(`Deleted ${result.deletedCount} transactions for user ${userId}`);
    ...
    return res.status(500).json({
      success: false,
      message: 'An error occurred while deleting transactions. Please try again.'
    });
  }
});
